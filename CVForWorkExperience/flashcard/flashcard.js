...
function SubmitFlashcard() {
  ...
  for (let i = 0; i < contentArray.length; i++) {
    ...
    if (compar.toUpperCase() == question.value.toUpperCase()) {
      ...
      if (
        confirm(
          "This action will replace one of your flashcards, is this acceptable?"
        )
      ) {
        console.log(":3");
        ...
        document.getElementById("flashquestion").innerHTML =
          contentArray[i].my_question;
        document.getElementById("flashanswer").innerHTML =
          contentArray[i].my_answer;
      } else {
        allow = false;
      }
    } else {
...
function replaceQandA() {
  try {
    ...
    document.getElementById("flashquestion").innerHTML =
      contentArray[searchindex].my_question;

    document.getElementById("flashanswer").innerHTML =
      contentArray[searchindex].my_answer;
    console.log("Answer logged", contentArray[searchindex].my_answer);
  } catch {
    alert(
      "A PICNIC error has occured, please input an integer within the range of the amount of items in the local storage array to resolve this"
    );
  }
} // Useful for Ersatz Enterprise project
...
function cardbefore() {
  try {
    ...
    console.log(
      "Question logged",
      contentArray.find((x) => x.my_question == "Question 1")
    );
    questionFind = contentArray.findIndex(
      (x) => x.my_question == document.getElementById("flashquestion").innerText
    );
    console.log(questionFind);
    questionFind = questionFind - 1;
    console.log(questionFind);
    document.getElementById("flashquestion").innerHTML =
      contentArray[questionFind].my_question;

    document.getElementById("flashanswer").innerHTML =
      contentArray[questionFind].my_answer;
    console.log("Answer logged", contentArray[questionFind].my_answer);
    hideanswer();
  } catch {
    alert(
      "A PICNIC error has occured, please ensure that you have a flashcard in this integer position"
    );
  }
} // Useful for Ersatz Enterprise project
...
function randomcardchoose(cycle) {
  try {
    ...
    if (rand == questionFind) {
      ...
      if (cycle == 300) {
        window.alert(
          "randomisation of selected flashcard aborted, please try adding more flashcards to remedy this error"
        );
      } else {
      ...
      document.getElementById("flashquestion").innerHTML =
        contentArray[rand].my_question;

      document.getElementById("flashanswer").innerHTML =
        contentArray[rand].my_answer;
      console.log("Answer logged", contentArray[rand].my_answer);
      hideanswer();
    }
  } catch {
    window.alert(
      "A PICNIC error has occured, to fix this please create a flashcard."
    );
  }
}
